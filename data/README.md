...
- **raw scraping and combination**
  ...
  - `csv_combiner.py`: helper to merge per-season or per-source csv files into a single combined csv used by the models

- **csv schema**
  - model code expects csv files with at least the following columns (matching `data/temp.csv`):
    ...
    - `Pace`, `ORtg`: optional additional stats not directly used by the hmm
